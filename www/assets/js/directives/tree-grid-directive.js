        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "   <thead>\n" +
                    "     <tr>\n" +
                    "       <th class='col-xs-12'><a ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</a><span ng-if=\"!expandingProperty.sortable\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span><i ng-if=\"expandingProperty.sorted\" class=\"{{expandingProperty.sortingIcon}} pull-right\"></i></th>\n" +
                    "       <th style='{{col.style}}' ng-repeat=\"col in colDefinitions\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
                    "     </tr>\n" +
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    "     <tr ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
                    "       ng-class=\"'level-' + {{ row.level }} + (row.branch.entrySelected ? ' active':'')\" class=\"tree-grid-row\" ng-click=\"on_user_click(row.branch)\">\n" +
                    "       <td style=\"white-space: normal\"><a><i style=\"{{get_user_firstcol_style(row.branch)}}\" ng-class=\"row.tree_icon\"\n" +
                    "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
                    "              class=\"indented tree-icon\"></i>\n" +
                    "           </a><span class=\"indented tree-label\" ng-style=\"get_user_firstcol_style(row.branch)\">\n" +
                    "             <div class='tree-grid-text' data-ng-bind-html=\"row.branch[expandingProperty.field] || row.branch[expandingProperty]\"></div></span>\n" +
                    "       </td>\n" +
                    "       <td style='{{col.style}}' ng-click=\"row_selected(row.branch)\" class=\"{{col.entrySelectable && row.branch['entrySelected'] ? 'treeGridSelected':''}}\" ng-repeat=\"col in colDefinitions\">\n" +
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        scope.get_user_firstcol_style = function (branch) {
                            return scope.getFirstcolStyle({
                                branch: branch
                            });
                        };

                        scope.row_selected = function (branch) {
                            /*
                            if (scope.onClick) {
                                scope.onClick({
                                    branch: branch
                        ...
                        var sort_by = function (field, descending, sortingType) {
                            var key = sortingType === "number" ? function (x) {
                                return parseFloat(x[field])
                            } : function (x) {
                                return (x[field] === null ? "" : x[field].toLowerCase())
                            };
                        ...
                        on_treeData_change = function () {
                            ...
                            add_branch_to_list = function (level, branch, visible) {
                                ...
                                if (!branch.children || branch.children.length === 0) {
                                    //TODO Leaf can be set by branch
                                    if (branch.iconLeaf) {
                                        tree_icon = branch.iconLeaf;
                                    } else {
                        ...
                        /*
                        //TODO NO AUTOMATIC EXPANDING
                        for_each_branch(function (b, level) {
                            b.level = level;
                            console.error("add_branch_to_list2:" + b.level + ", " + expand_level);
